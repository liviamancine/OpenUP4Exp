...
function set$1(target, path, value) {
	...
	forEach(path, function(key, idx) {
		...
		if (isUndefined$1(nextKey)) if (isUndefined$1(value)) delete currentTarget[key];
...
function getExternalLabelMid(element) {
	...
	else if (is$1(element, "bpmn:Group")) return {
...
function getLabelAttr(semantic) {
	if (is$1(semantic, "bpmn:FlowElement") || is$1(semantic, "bpmn:Participant") || is$1(semantic, "bpmn:Lane") || is$1(semantic, "bpmn:SequenceFlow") || is$1(semantic, "bpmn:MessageFlow") || is$1(semantic, "bpmn:DataInput") || is$1(semantic, "bpmn:DataOutput")) return "name";
	if (is$1(semantic, "bpmn:TextAnnotation")) return "text";
	if (is$1(semantic, "bpmn:Group")) return "categoryValueRef";
}
/**
* @param {ModdleElement} semantic
*
...
function getBBox(elements, stopRecursion) {
	...
	if (!isArray$2(elements)) elements = [elements];
...
//#endregion
//#region node_modules/tiny-svg/dist/index.js
...
ClassList$1.prototype.remove = function(name) {
	if ("[object RegExp]" == toString$1.call(name)) return this.removeMatching(name);
...
var node = null;
function getNode() {
	if (node === null) node = create$1("svg");
	return node;
}
...
function createMatrix(a, b, c, d, e, f) {
	...
	switch (arguments.length) {
		...
		case 1: return extend$1(matrix, a);
		case 6: return extend$1(matrix, {
			a,
			b,
			c,
			d,
...
function updateLine(gfx, points) {
	attr$1(gfx, { d: componentsToPath(drawPath(points, parseInt(gfx.dataset.cornerRadius, 10) || 0)) });
	return gfx;
}
//#endregion
	...
	*/
	...
	var eventIconRenderers = {
		...
		"bpmn:TimerEventDefinition": function(parentGfx, element, attrs = {}) {
			...
			var circle = drawCircle(parentGfx, baseWidth, baseHeight, .2 * baseHeight, {
	...
	function renderEventIcon(element, parentGfx, attrs = {}, proxyElement) {
		...
		if (semantic.get("eventDefinitions") && semantic.get("eventDefinitions").length > 1) if (semantic.get("parallelMultiple")) return eventIconRenderers["bpmn:ParallelMultipleEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		else return eventIconRenderers["bpmn:MultipleEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:MessageEventDefinition")) return eventIconRenderers["bpmn:MessageEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:TimerEventDefinition")) return eventIconRenderers["bpmn:TimerEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:ConditionalEventDefinition")) return eventIconRenderers["bpmn:ConditionalEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:SignalEventDefinition")) return eventIconRenderers["bpmn:SignalEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:EscalationEventDefinition")) return eventIconRenderers["bpmn:EscalationEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:LinkEventDefinition")) return eventIconRenderers["bpmn:LinkEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:ErrorEventDefinition")) return eventIconRenderers["bpmn:ErrorEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:CancelEventDefinition")) return eventIconRenderers["bpmn:CancelEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:CompensateEventDefinition")) return eventIconRenderers["bpmn:CompensateEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		if (isTypedEvent(semantic, "bpmn:TerminateEventDefinition")) return eventIconRenderers["bpmn:TerminateEventDefinition"](parentGfx, nodeElement, attrs, isThrowing);
		return null;
	}
	...
	function renderTaskMarkers(parentGfx, element, taskMarkers = [], attrs = {}) {
		...
		if (semantic.get("isForCompensation")) taskMarkers.push("CompensationMarker");
		if (is$1(semantic, "bpmn:AdHocSubProcess")) {
			taskMarkers.push("AdhocMarker");
			if (!subprocess) assign$1(attrs, { compensation: attrs.compensation - 18 });
		}
		...
		if (loopCharacteristics) {
			...
			if (taskMarkers.includes("AdhocMarker")) assign$1(attrs, {
				seq: -23,
				loop: -18,
				parallel: -24
			});
			if (isSequential === void 0) taskMarkers.push("LoopMarker");
			if (isSequential === false) taskMarkers.push("ParallelMarker");
			if (isSequential === true) taskMarkers.push("SequentialMarker");
		}
		if (taskMarkers.includes("CompensationMarker") && taskMarkers.length === 1) assign$1(attrs, { compensation: -8 });
		forEach(taskMarkers, function(marker) {
			renderTaskMarker(marker, parentGfx, element, attrs);
		});
	}
	function renderLabel(parentGfx, label, attrs = {}) {
		attrs = assign$1({ size: { width: 100 } }, attrs);
	...
	function renderExternalLabel(parentGfx, element, attrs = {}) {
		...
		return renderLabel(parentGfx, getLabel(element), {
			box,
			style: assign$1({}, textRenderer.getExternalStyle(), { fill: getLabelColor(element, defaultLabelColor, defaultStrokeColor, attrs.stroke) })
		});
	}
	...
	function renderSubProcess(parentGfx, element, attrs = {}) {
		...
		if (isEventSubProcess(element)) {
			...
			if (!expanded) {
				var startEvents = (getBusinessObject(element).flowElements || []).filter((e) => is$1(e, "bpmn:StartEvent"));
				if (startEvents.length === 1) renderEventSubProcessIcon(startEvents[0], parentGfx, attrs, element);
			}
		}
...
BpmnRenderer.prototype.getShapePath = function(shape) {
	if (isLabel(shape)) return getRoundRectPath(shape, EXTERNAL_LABEL_BORDER_RADIUS);
...
function parsePadding(padding) {
	if (isObject(padding)) return assign$1({
		top: 0,
		left: 0,
		right: 0,
		bottom: 0
...
function getCanvasContext() {
	if (!_canvasContext) _canvasContext = document.createElement("canvas").getContext("2d");
...
function buildFont(style) {
	...
	if (style.fontStretch) parts.push(style.fontStretch);
...
function buildLength(value) {
	...
	if (typeof value === "number" || /^-?\d+(\.\d+)?$/.test(value)) return value + "px";
	return value;
}
/**
* @param {string} text
...
function getTextBBox(text, style) {
	...
	if ("letterSpacing" in ctx) ctx.letterSpacing = buildLength(style.letterSpacing) || "0px";
	var emptyLine = text === "";
...
Text.prototype.layoutText = function(text, options) {
	...
	while (lines.length) layouted.push(layoutNext(lines, maxWidth, style));
...
function TextRenderer(config) {
	...
	var externalStyle = assign$1({}, defaultStyle, { fontSize: parseInt(defaultStyle.fontSize, 10) - 1 }, config && config.externalStyle || {});
	...
	*/
	this.getExternalLabelBounds = function(bounds, text) {
		var dimensions = getTextboxDimensions(text, {
	...
	*/
	function getTextboxDimensions(text, box, layoutOptions) {
		return textUtil.getDimensions(text, assign$1({ box }, layoutOptions));
	}
	/**
...
*/
var translate_default = { translate: ["value", translate] };
//#endregion
//#region node_modules/path-intersection/intersect.js
var math = Math;
math.PI;
math.min;
math.max;
...
Overlays.prototype.remove = function(filter) {
	...
	if (!isArray$2(overlays)) overlays = [overlays];
	var self = this;
...
Overlays.prototype._addOverlay = function(overlay) {
	...
	if (isString(html)) html = domify(html);
...
CommandInterceptor.prototype.on = function(events, hook, priority, handlerFn, unwrap, that) {
	...
	if (!isArray$2(events)) events = [events];
	var eventBus = this._eventBus;
...
function DrilldownOverlayBehavior(canvas, eventBus, elementRegistry, overlays, translate) {
	...
	this.executed([
		...
		"shape.move",
		"shape.delete"
	], LOW_PRIORITY$1, function(context) {
...
function InteractionEvents(eventBus, elementRegistry, styles) {
	...
	*/
	this.removeHits = function(gfx) {
		forEach(all(".djs-hit", gfx), remove$2);
	};
	/**
...
Selection.prototype.select = function(elements, add) {
	var oldSelection = this.get();
	if (!isArray$2(elements)) elements = elements ? [elements] : [];
	var canvas = this._canvas;
	var rootElement = canvas.getRootElement();
	elements = elements.filter(function(element) {
		return rootElement === canvas.findRoot(element);
	});
	if (add) forEach(elements, (element) => {
		this._selectedElements.add(element);
	});
	else this._selectedElements = new Set(elements);
	this._eventBus.fire("selection.changed", {
		oldSelection,
		newSelection: this.get()
	});
...
function SelectionVisuals(canvas, eventBus) {
	...
	eventBus.on("selection.changed", function(event) {
		...
		var oldSelection = new Set(event.oldSelection), newSelection = new Set(event.newSelection);
		for (let e of oldSelection) if (!newSelection.has(e)) deselect(e);
		for (let e of newSelection) if (!oldSelection.has(e)) select(e);
	});
}
...
function SelectionBehavior(eventBus, selection, canvas, elementRegistry) {
	eventBus.on("create.end", 500, function(event) {
		...
		if (canExecute) {
			...
			if (isArray$2(autoSelect)) selection.select(autoSelect);
...
DefaultRenderer.prototype.drawShape = function drawShape(visuals, element, attrs) {
	...
	if (isFrameElement$1(element)) attr$1(rect, assign$1({}, this.FRAME_STYLE, attrs || {}));
	else attr$1(rect, assign$1({}, this.SHAPE_STYLE, attrs || {}));
	append(visuals, rect);
	return rect;
};
/**
...
function Styles() {
	...
	*/
	this.cls = function(className, traits, additionalAttrs) {
		return assign$1(this.style(traits, additionalAttrs), { "class": className });
	};
	/**
...
Canvas.prototype._init = function(config) {
	...
	if (config.deferUpdate) this._viewboxChanged = debounce(this._viewboxChanged, 300);
	/**
...
Canvas.prototype.findRoot = function(element) {
	...
	for (const element of elements) {
		...
		if (!plane) return;
		if (root && plane.rootElement !== root) return;
...
Canvas.prototype._resolveElement = function(element) {
	if (typeof element === "string") return this._elementRegistry.get(element);
	return element;
};
/**
...
Canvas.prototype.scrollToElement = function(element, padding) {
	...
	const elementBounds = getBBox(visibleElements), elementTrbl = asTRBL(elementBounds), viewboxBounds = this.viewbox(), zoom = this.zoom();
	let dx, dy;
...
GraphicsFactory.prototype.updateContainments = function(elements) {
	...
	forEach(reduce(elements, function(map, e) {
		...
		forEach(children, function(child) {
			...
			if (childGfx === expected) expected = expected.nextSibling;
			else childrenGfx.insertBefore(childGfx, expected);
		});
	});
};
/**
...
//#endregion
//#region node_modules/moddle/dist/index.js
/**
* Moddle base element.
*/
function Base() {}
...
Registry.prototype.registerType = function(type, pkg) {
	...
	var ns = parseName(type.name, pkg.prefix), name = ns.name, propertiesByName = Object.create(null);
...
//#endregion
//#region node_modules/saxen/dist/index.js
...
function buildNsMatrix(nsUriToPrefix) {
	var nsMatrix = Object.create(null), uri, prefix;
...
function Parser(options) {
	...
	*/
	this["parse"] = function(xml) {
		...
		if (streaming) throw error$1("parse during stream; call end() first");
		resetState();
		parse(xml);
		getContext = noopGetContext;
		parseStop = false;
		...
		while (j !== -1) {
			...
			if (isNamespace) {
				...
				if (elNameCacheMatrix !== nsMatrix) {
					...
					if (elNameCache === void 0) elNameCache = nsMatrix[NAME_CACHE] = Object.create(null);
...
//#endregion
//#region node_modules/moddle-xml/dist/index.js
...
function normalizeTypeName(name, nsMap, model) {
	...
	const normalizedNameNs = parseName(`${nsMap[nameNs.prefix] || nameNs.prefix}:${nameNs.localName}`);
	return prefixedToName(normalizedNameNs, model.getPackage(normalizedNameNs.prefix));
}
...
ElementHandler.prototype.getPropertyForNode = function(node) {
	...
	if (property && !property.isAttr) {
		...
		if (serializationType) {
			...
			if (elementTypeName) {
				...
				return assign$1({}, property, { effectiveType: getModdleDescriptor(elementType).name });
			}
		}
		return property;
	}
	...
	if (pkg) {
		...
		if (property) return assign$1({}, property, { effectiveType: getModdleDescriptor(elementType).name });
	} else {
		property = find(descriptor.properties, function(p) {
...
Namespaces.prototype.getUsed = function(ns) {
	return [].concat(this.wellknown, this.custom).filter((ns) => {
		var nsKey = this.getNSKey(ns);
...
function getElementNs(ns, descriptor) {
	if (descriptor.isGeneric) return assign$1({ localName: descriptor.ns.localName }, ns);
	else return assign$1({ localName: nameToAlias(descriptor.ns.localName, descriptor.$pkg) }, ns);
}
...
ElementSerializer.prototype.nsAttributeName = function(element) {
	...
	else return assign$1({ localName: ns.localName }, effectiveNs);
};
...
ElementSerializer.prototype.parseGenericContainments = function(element) {
	...
	if (body) this.body.push(new BodySerializer().build({ type: "String" }, body));
	var children = element.$children;
	if (children) forEach(children, (child) => {
	...
	forEach(properties, function(p) {
		...
		if (p.isBody) body.push(new BodySerializer().build(p, value[0]));
			...
			forEach(value, function(v) {
				var serializer;
				if (serialization) if (serialization === SERIALIZE_PROPERTY) serializer = new ElementSerializer(self, p);
...
ElementSerializer.prototype.logNamespaceUsed = function(ns, local) {
	...
	if (!ns && !prefix) ns = this.logNamespace({ uri }, wellknownUri === uri, true);
	if (!ns) {
		newPrefix = prefix;
		idx = 1;
...
//#endregion
//#region node_modules/bpmn-moddle/dist/index.js
/**
...
function SimpleBpmnModdle(additionalPackages, options) {
	return new BpmnModdle(assign$1({}, packages, additionalPackages), options);
}
//#endregion
...
BaseViewer.prototype._createModdle = function(options) {
	return new SimpleBpmnModdle(assign$1({}, this._moddleExtensions, options.moddleExtensions));
};
...
function addProjectLogo(container) {
	const linkElement = domify("<a href=\"http://bpmn.io\" target=\"_blank\" class=\"bjs-powered-by\" title=\"Powered by bpmn.io\" >" + BPMNIO_IMG + "</a>");
